# Dictionary Rename Window

get lib style-tag

tell '.rename-dict-form' [
 object [
  padding 20px
 ]
]

tell '.rename-dict-form label' [
 object [
  display block
  margin-bottom 8px
  color '#e0e0d0'
...
tell '.rename-dict-form input:focus' [
 object [
  outline none
  border-color '#4a9eff'
 ]
...
tell '.rename-dict-form-error.visible' [
 object [
  display block
 ]
]

tell '.rename-dict-form button' [
 object [
...
tell '.rename-dict-form button:hover' [
 object [
  background-color '#555558'
 ]
]

tell '.rename-dict-form button.primary' [
 object [
...
tell '.rename-dict-form button.primary:hover' [
 object [
  background-color '#5aafff'
 ]
]

tell '.rename-dict-form-id' [
 object [
  color '#808080'
  font-size 12px
  margin-bottom 12px
...
get conductor register, call dictionary:rename [
 function arg [
  ...
  get dict-id, false [
   log No dictionary to rename
   value undefined
  ]

  set name-ref [ object [ name 'Untitled Dictionary' ] ]
  try [
   set dict [ get dict-service fetch-dictionary, call [ get dict-id ] ]
   get dict name, true [
    set name-ref name [ get dict name ]
   ]
  ] [
   # Failed to fetch, use default
  ]
  set dict-name [ get name-ref name ]

  set rename-window [
   get components window, call 'Rename Dictionary' 220 350
  ]
  ...
  get form classList add, call rename-dict-form

  set dict-id-el [ global document createElement, call div ]
  get dict-id-el classList add, call rename-dict-form-id
  set dict-id-el textContent [ template 'Dictionary ID: %0' [ get dict-id ] ]
  get form appendChild, call [ get dict-id-el ]

  set label [ global document createElement, call label ]
  set label textContent 'Dictionary name:'
  get form appendChild, call [ get label ]

  set input [ global document createElement, call input ]
  set input type text
  set input value [ get dict-name ]
  set input placeholder 'Enter dictionary name'
  get form appendChild, call [ get input ]

  set error-msg [ global document createElement, call div ]
  get error-msg classList add, call rename-dict-form-error
  get form appendChild, call [ get error-msg ]

  set buttons [ global document createElement, call div ]
  get buttons classList add, call rename-dict-form-buttons
  ...
  get save-btn addEventListener, call click [
   function [
    ...
    get new-name length, > 0, true [
     ...
     try [
      set response [ get dict-service rename-dictionary, call [ get dict-id ] [ get new-name ] ]
      get response, true [
       set rename-result success true
      ], false [
       set rename-result error 'Failed to rename dictionary'
      ]
     ] [
